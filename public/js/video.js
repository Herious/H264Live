...
function decode_frame(ws_addr) {
    ...
    var cb = function (addr_y, addr_u, addr_v, stride_y, stride_u, stride_v, width, height, pts, index) {
    ...
    var videoCallback = Module.addFunction(cb, 'viiiiiiiiii');

    var handle = Module._openDecoder(decoder_type, videoCallback, LOG_LEVEL_WASM)
    if(handle != 0) {
        console.log("deocde frame openDecoder success");
    } else {
        console.error("decode frame openDecoder failed with error");
        return;
    }
    ...
    ws.onmessage = function (ev) {
        if (0 == handle) {
            console.log('initialized not ready');
            return;
        }
        if (ev.data instanceof Object) { 
            ev.data.arrayBuffer().then(buffer => {
                ...
                Module._decodeOneFrame(handle, Module.cacheBuffer, size, pts++, 0);
            });
        } 
        else {
            // return
    ...
    ws.onclose = function () {
        ...
        Module._flushDecoder(handle);
        Module._closeDecoder(handle);
    };
}
