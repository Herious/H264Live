...
function decode_frame_worker(ws_addr, canvasList, index) {
    var ws = new WebSocket(ws_addr);
    var decodeWorker = new Worker("js/video_worker.js");
    ws.onopen = function () {
        decodeWorker.send
    }
    var worker = new Worker("js/fsdraw.js");
    var fsCanvas = document.getElementById('rects-canvas');
    var offscreen = fsCanvas.transferControlToOffscreen();
    worker.postMessage({ data: 'init', canvas: offscreen }, [offscreen]);
    ws.onmessage = function (ev) {
        if (ev.data instanceof Object) {
            ...
            worker.postMessage({ "data": ev.data});
        }
...
function decode_frame(ws_addr, canvasList, index) {
    setDecoder(0);
    var ws = new WebSocket(ws_addr);
    var pts = 0;
    ...
    ws.onmessage = function (ev) {
        ...
        if((nowTime - lprStartTime) > 3 * 1000) {
...
function displayYUVFrame(obj, canvasList, index) {
    ...
    console.log(webglPlayers[index])
